...
BEGIN
    ...
    WHILE (i < (SELECT MAX(id_paciente) FROM pacientes)) DO
        IF ((SELECT email FROM pacientes WHERE id_paciente = (i + 1)) LIKE f_email) THEN
            RETURN 1;
        END IF;
        SET i = i + 1;
    END WHILE;
...
BEGIN
    ...
    WHILE (i < (SELECT MAX(id_doctor) FROM doctores)) DO
        IF ((SELECT email FROM doctores WHERE id_doctor = (i + 1)) LIKE f_email) THEN
            RETURN 1;
        END IF;
        SET i = i + 1;
    END WHILE;
...
BEGIN
    ...
    WHILE (i < (SELECT MAX(id_medicamento) FROM medicamentos)) DO
        IF ((SELECT nombre FROM medicamentos WHERE id_medicamento = (i + 1)) LIKE f_nombreMedicamento) THEN
            RETURN 1;
        END IF;
        SET i = i + 1;
    END WHILE;
